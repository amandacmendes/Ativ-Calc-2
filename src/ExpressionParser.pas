...
const
  ...
  FuncNames: array [1..MaxFuncs] of
    ShortString = ('SEN', 'COS', 'TAN', 'RAIZ', 'QUADRADO', 'CUBO', 'ROUND', 'TRUNC', 'ABS',
    'FRAC', 'PAR', 'LN',
    'LOG2', 'LOG10', 'ARCSEN', 'ARCCOS', 'ARCTAN', 'COTAN',
    'DEGRAD', 'RADDEG');
...
begin
  ...
  if Level in [0, 1] then
    Result := Result + 'A J U D A (' + IntToStr(Level) + '): Parse' + BR +
  ...
  if Level in [0, 2] then
    Result := Result + 'A J U D A (' + IntToStr(Level) + '): ParseF' + BR +
      'Devolve em formato alfanumérico (ShortString) o valor da expressão passada por parâmetro!' +
      BR +
      'Uso: <string> := Parse(Expressão:ShortString;Precisão,Decimais:Integer);        Exemplo: s := ParseF(''1+2+3'',3,2);'
      + BR +
      'Precisão: Número mínimo de algarismos reservados para o resultado'
      +
      BR +
      'Decimais: Número máximo de casas decimais para o resultado' + BR + BR;

  if Level in [0, 3] then
    Result := Result + 'A J U D A (' + IntToStr(Level) + '): History' + BR +
      'Grupo de funções para gerir o histórico de operações efectuadas:'
      +
      BR + '- Inicializar o Histórico: Uso: HistoryClear;'
      + BR +
  ...
  if Level in [0, 4] then
    Result := Result + 'A J U D A (' + IntToStr(Level) + '): Parêntesis' + BR +
  ...
  if Level in [0, 5] then
    Result := Result + 'A J U D A (' + IntToStr(Level) + '): Constantes' + BR +
      'As constantes são representações alfabéticas de um valor numérico fixo. Podem ser inseridas normalmente nas expressões.' + BR + 'Uso: CONSTANTE   ***   Coloca-se o nome da constante em qualquer lugar da expressão' + BR + '- PI: 3.14159265' + BR + '- E: 2.71828182' + BR + BR;

  if Level in [0, 6] then
    Result := Result + 'A J U D A (' + IntToStr(Level) +
      '): Funções Especiais (múltiplos argumentos)' + BR +
      'As funções especiais recebem múltiplos argumentos. Podem ser inseridas normalmente nas expressões, mas não é possível encadear funções especiais, embora possam ser usadas funções normais dentro das especiais.' + BR + 'Uso: FUNÇÃO [arg1 arg2 arg3 ... arg_n]   ***   Coloca-se o nome da função, seguido dos argumentos, separados por espaços e contidos entre parêntesis rectos [].' + BR + '- MEDIA: Calcula a média de todos os argumentos fornecidos.' + BR + '- HIPOTENUSA: Calcula a hipotenusa de um triângulo rectângulo, cujos catetos são os argumentos 1 e 2.' + BR + '- DIV: Calcula a divisão inteira dos dois primeiros argumentos.' + BR + '- MOD: Calcula o resto da divisão inteira dos dois primeiros argumentos.' + BR + BR;

  if Level in [0, 7] then
    Result := Result + 'A J U D A (' + IntToStr(Level) + '): Funções (argumento único)' + BR +
      'As funções recebem um único argumento. Podem ser inseridas normalmente nas expressões, e podem ser encadeadas desde que usem parêntesis em redor das expressões internas Ex.: ABS (SEN 5) .' + BR + 'Uso: FUNÇÃO Arg  ou  FUNÇÃO (Expressão)   ***   Coloca-se o nome da função, seguido do argumento. O Argumento pode ser uma expressão também, se contido entre parêntesis curvos.' + BR + '- SEN: Calcula o seno de um ângulo (em radianos).' + BR + '- COS: Calcula o coseno de um ângulo (em radianos).' + BR + '- TAN: Calcula a tangente de um ângulo (em radianos).' + BR + '- ARCSEN: Calcula o arcoseno de um ângulo (em radianos).' + BR + '- ARCCOS: Calcula o arcocoseno de um ângulo (em radianos).' + BR + '- ARCTAN: Calcula a arcotangente de um ângulo (em radianos).' + BR + '- COTAN: Calcula a cotangente de um ângulo (em radianos).' + BR + '- DEGRAD: Converte graus em radianos.' + BR + '- RADDEG: Converte radianos em graus.' + BR + '- RAIZ: Calcula a raíz quadrada.' + BR + '- QUADRADO: Calcula o quadrado de um valor.' + BR + '- CUBO: Calcula o cubo de um valor.' + BR + '- ROUND: Arredonda um valor ao seu inteiro mais próximo.' + BR + '- TRUNC: Trunca um valor ao número inteiro imediatamente inferior.' + BR + '- ABS: Devolve a parte absoluta de um valor (sem sinal negativo ou positivo).' + BR + '- FRAC: Devolve a parte fraccional de um valor.' + BR + '- PAR: Devolve 1 se um valor for par, ou 0 se o valor for impar.' + BR + '- LN: Calcula o logaritmo natural de um valor.' + BR + '- LOG2: Calcula o logaritmo de base 2 de um valor.' + BR + '- LOG10: Calcula o logaritmo de base 10 de um valor.' + BR + BR;

  if Level in [0, 8] then
    Result := Result + 'A J U D A (' + IntToStr(Level) + '): Operadores Lógicos' + BR +
      'Os operadores lógicos devolvem 1 ou 0 caso a operação retorne True ou False. .'
      +
      BR +
      'Uso: arg1 OPERADOR arg2   ***   Coloca-se o operador entre dois argumentos.' + BR +
      'Prioridade ente operadores: Máxima' + BR +
      '- =: True se os argumentos tiverem o mesmo valor.'
      +
      BR +
      '- !: True se os argumentos tiverem valor diferente.' + BR +
      '- <: True se o primeiro argumento for menor que o segundo.'
      +
      BR +
      '- >: True se o primeiro argumento for maior que o segundo.' + BR + BR;

  if Level in [0, 9] then
    Result := Result + 'A J U D A (' + IntToStr(Level) + '): Operadores Aritméticos' + BR +
  ...
  if Level in [0, 10] then
    Result := Result + 'A J U D A (' + IntToStr(Level) + '): Cálculos de Sinais' + BR +
      'Os sinais duplos são calculados de acordo com as leis matemáticas:'
      +
      BR + '- ++ = +' + BR +
      '- +- = -' + BR +
      '- -+ = -' + BR +
  ...
  try
    case NumSFunc of
      ...
      begin // DIV
        ...
        Value := IntToStr(Round(StrToFloat(Param)) div
          Round(StrToFloat(Value)));
      end;
      4:
      begin // MOD
        Value := Param + ' ';
        ...
        Delete(Value, 1, pos(' ', Value));
        Delete(Value, pos(' ', Value), 999);
        Value := IntToStr(Round(StrToFloat(Param)) mod
          Round(StrToFloat(Value)));
      end;
    end;
  except
...
begin
  ...
  try
    case NumFunc of
      ...
      11: if Odd(Trunc(GetNextExpressionValue(Expression, FuncStart))) then
          Value := '1'
        else
          Value := '0';
...
begin
  ...
  if OpPos > length(expression) then
  begin
    ...
    begin
      ...
      repeat
        Inc(OpPos);
      until (Expression[OpPos] in ['*', '/', 'x', 'X', ':']) or
        (OpPos > length(Expression));
      ...
      begin
        ...
        repeat
          Inc(OpPos);
        until (Expression[OpPos] in ['+']) or
          (OpPos > length(Expression));
        if OpPos > length(expression)
        then
          exit                       // Se não encontrar nenhum operador
        else
          Op := Expression[OpPos];     // Guarda qual o operador de prioridade 3 encontrado
      end
      else
        Op := Expression[OpPos];
      // Guarda qual o operador de prioridade 2 encontrado
    end
    else
      Op := Expression[OpPos];
    // Guarda qual o operador de prioridade 1 encontrado
  end
  else
    Op := Expression[OpPos];
  // Guarda qual o operador de prioridade 0 encontrado
  ...
  try
    case Op of
      '=': if GetLastExpressionValue(Expression, OpPos) =
          GetNextExpressionValue(Expression, OpPos) then
          Value := '1'
        else
          Value := '0';
      '!': if GetLastExpressionValue(Expression, OpPos) <>
      ...
      '>': if GetLastExpressionValue(Expression, OpPos) >
          ...
          Value := '0';
      '^': Value := FloatToStr(
          Power(GetLastExpressionValue(Expression, OpPos), GetNextExpressionValue(
          Expression, OpPos)));
      '*', 'x', 'X': Value :=
          FloatToStr(GetLastExpressionValue(Expression, OpPos) * GetNextExpressionValue(
          Expression, OpPos));
      '/', ':': Value := FloatToStr(GetLastExpressionValue(
          Expression, OpPos) / GetNextExpressionValue(Expression, OpPos));
      '+': Value := FloatToStr(GetLastExpressionValue(Expression, OpPos) +
          GetNextExpressionValue(Expression, OpPos));
      '-': Value := FloatToStr(GetLastExpressionValue(Expression, OpPos) -
          GetNextExpressionValue(Expression, OpPos));
    end;
  except
...
begin
  ...
  Value := '';
  i := 1;
  while (i <= Length(Expression)) and (Expression[i] in
      ['0'..'9', '-', DecimalSeparator]) do
  begin
  ...
  begin
    case Expression[i] of
      ...
      '+': case Expression[i + 1] of
          ...
          begin
            pExpr := pExpr + '+-';
            // mantém ambos, pois o - pertence ao valor
            Inc(i, 2);
          end;
          else
          begin
            pExpr := pExpr + '+';
            // Se for um + simples, mantém-no
            Inc(i, 1);
          end;
        end;
      // Se o caracter encontrado for o sinal -
      '-': case Expression[i + 1] of
          ...
          begin
            pExpr := pExpr + '+-';
            // Inverte os sinais, e o - passa a pertencer ao valor seguinte
            Inc(i, 2);
          end;
          '-':
          begin
          ...
          begin
            if i > 1 then
              pExpr := pExpr + '+-'
            // Se não for o sinal de um 1º valor negativo, converte-se em +-, onde o - passa a pertencer ao valor e o + é a operação
            else
              pExpr := pExpr + '-';
            Inc(i, 1);
          end;
      ...
      begin
        pExpr := pExpr + Expression[i];
        // Adiciona qualquer outro caracter que não um sinal de + ou -
        Inc(i, 1);
      end;
    end;
  end;
...
begin
  ...
  if Result > 1 then
    if Upcase(Str[Result - 1]) in ['A'..'Z'] then
      Result := 0;
end;
